    ...
    var body: some View {
        HStack {
            VStack(alignment: .leading, spacing: 3) {
                HStack {
                    Text(
                        event.schedule?.rooms[session.room]?.localized().name
                            ?? session.room
                    )
                    .font(.caption2)
                    .padding(.vertical, 1)
            ...
            .opacity(
                session.end.isBeforeDate(
                    DateInRegion(), orEqual: true, granularity: .minute)
                    && dimPastSession ? pastSessionOpacity : 1)
            Spacer()
